...
import { BookOpen, Calendar, Share2, ArrowRight, Quote } from "lucide-react";
...
export default function DailyDevotional({ post }: { post?: any }) {
	...
	return (
		<section className="py-10 bg-gradient-to-b from-white to-gray-50 dark:from-gray-950 dark:to-gray-900">
			<div className="container-custom">
				...
				>
					<Badge
						variant="outline"
						className="mb-4 px-4 py-1 text-2xl border-primary/20 text-primary bg-primary/5"
					>
						Spiritual Nourishment
					</Badge>
					<h2 className="section-title mb-4">Word for Today</h2>
					<p className="section-subtitle max-w-2xl mx-auto">
						Daily insights to strengthen your walk with Christ and empower your
						spiritual journey.
					</p>
				</motion.div>
						...
						<div className="grid md:grid-cols-[1.5fr,1fr] lg:grid-cols-[2fr,1fr]">
							<div className="p-8 md:p-10 flex flex-col justify-between relative order-2 md:order-1">
								...
								<div className="relative">
									<div className="flex items-center gap-3 text-sm text-muted-foreground mb-6">
										<div className="flex items-center gap-1.5">
											...
											<span>
												{format(new Date(displayData.date), "MMMM d, yyyy")}
											</span>
										</div>
								...
								<div className="flex flex-wrap items-center gap-4 pt-4 relative">
									<Link
										href={post ? `/posts/${post.id}` : "/devotionals"}
							...
							<div
								className={`relative flex flex-col justify-center order-1 md:order-2 ${displayData.coverImage ? "p-0" : "bg-primary/5 dark:bg-primary/10 p-8 md:p-10 border-b md:border-b-0 md:border-l border-primary/10"}`}
							>
