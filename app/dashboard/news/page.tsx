    ...
    <div>
      ...
      {showForm && (
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <Card>
            <CardHeader><CardTitle>{isEditing ? "Edit News" : "Create News"}</CardTitle></CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <Textarea id="content" value={formData.content} onChange={(e) => setFormData({ ...formData, content: e.target.value })} rows={8} required />
                </div>
                <div>
                ...
                <Button type="submit">{isEditing ? "Update News" : "Create News"}</Button>
              </form>
            </CardContent>
          </Card>
        </motion.div>
          ...
          {news.map((item, index) => (
            <motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
              <Card className="hover:shadow-lg transition-shadow overflow-hidden">
                <Link href={`/news/${item.id}`} className="block cursor-pointer">
                    {item.coverImage && (
                    ...
                    <div className="flex justify-between items-start">
                        ...
                        <p className="text-sm text-gray-600 mt-2">{formatDate(item.createdAt)}</p>
                        {item.excerpt && <p className="text-sm text-gray-700 mt-2">{item.excerpt}</p>}
                        </div>
                        <span className={`px-3 py-1 rounded-full text-sm ${item.published ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>
